...
def get_dir(cur_path: str, level: int = 0) -> int:
    path = Path(cur_path)
    files = []
    dirs = []

    for x in path.iterdir(): #формуємо окремо список з файлами і окремо з каталогами, щоб файли вивести в першу чергу, до рекурсії
        if x.exists():
            files.append(x) if x.is_file() else dirs.append(x)

    print_dirs(path.name+"/", level)
    print_files(files, level+1)

    if len(dirs):
        [get_dir(dir, level+1) for dir in dirs]
    else:
        return 0
...
def print_files(filenames: list[Path], level: int):
    spaces = level * num_tabs * " "
    for filename in filenames:
        print(Fore.YELLOW + spaces+filename.name)
    print(Style.RESET_ALL, end='')
